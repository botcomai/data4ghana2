document.getElementById("adminLoginForm").addEventListener("submit", async function(e) {
    ...
    try {
        ...
        if (dbError) throw new Error("Could not verify administrative roles. Core database failure.");

        // DENY ACCESS IF NOT ADMIN OR SUPER AGENT
        const allowedRoles = ['admin', 'super_agent'];
        if (!dbData || !allowedRoles.includes(dbData.role)) {
            // Immediately sign them back out
            await window.supabase.auth.signOut();
            throw new Error("Authorization Denied. This terminal is strictly for Data4Ghana Administrative Staff.");
        }

        // Access Granted
        const roleLabel = dbData.role === 'super_agent' ? 'Super Agent' : 'Administrator';
