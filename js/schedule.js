...
function renderStats() {
  ...
  if (allOrders.length === 0) { statsEl.style.display = 'none'; return; }
  ...
  const multiples  = allOrders.filter(o => o.note?.includes('Multiple')).length;
  ...
  document.getElementById('statMultiple').innerText = multiples;
}
  ...
  if (filteredOrders.length === 0) {
    container.innerHTML = `
      <div class="sched-empty">
        ...
        <p>${allOrders.length === 0 ? 'No scheduled orders yet. Bulk orders with duplicate pending entries will appear here.' : 'No orders match your filter.'}</p>
      </div>`;
    return;
  }

  const rows = filteredOrders.map(o => {
    const date     = new Date(o.scheduled_at).toLocaleString();
    const isMulti  = o.note?.includes('Multiple');
    const badgeClass = isMulti ? '' : 'scheduled';
    const badgeText  = isMulti ? '📋 Multiple Order' : '📅 Scheduled';
    ...
    return `
      <div class="sched-row" data-id="${o.id}">
        ...
        <div class="row-network">${o.network}</div>
        <div class="row-plan">${o.plan}</div>
        <div class="row-amount">₵${parseFloat(o.amount).toFixed(2)}</div>
        <div class="row-type"><span class="type-badge ${badgeClass}">${badgeText}</span></div>
        <div class="row-date">${date}</div>
        <div class="row-actions">
          <button class="row-btn process" onclick="processSingle('${o.id}')">▶ Process</button>
          <button class="row-btn delete"  onclick="deleteSingle('${o.id}')">🗑</button>
        </div>
      </div>`;
  }).join('');
